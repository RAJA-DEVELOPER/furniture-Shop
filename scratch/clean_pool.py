...
# Pool of 130+ DISTINCT Unsplash photo IDs
POOL = [
    # Index (15)
    ...
    "photo-1631679706909-1844bbd07221", # login.html
    "photo-1631679707166-512c19a997d4", # signup.html
    "photo-1600585154340-be6161a56a0c_terms", # terms.html -> replace below
    "photo-1600607687939-ce8a6c25118c_privacy", # privacy.html -> replace below
    "photo-1600566753190-17f0baa2a6c3_sitemap", # sitemap.html -> replace below
    "photo-1600596542815-ffad4c1539a9_openplan", # open-plan-zoning.html
    "photo-1600607687644-c7171b42498f_office", # home-office-design.html
    "photo-1600585154526-990dced4db0d_sustainable", # sustainable-furniture.html
    "photo-1618219908412-a29a1bb7b86e_velvet", # velvet-upholstery-care.html
    "photo-1600573472591-ee6b68d14c68_bedroom", # bedroom-colour-palettes.html
    "photo-1600585152220-90363fe7e115_dining", # dining-table-guide.html

    # CSS home.css (6)
    "photo-1567496898669-ee935f5f647a_css1",
    "photo-1600607687920-4e2a09cf159d_css2",
    "photo-1581291518633-83b4ebd1d83e_css3",
    "photo-1544457070-4cd773b4d71e_css4",
    "photo-1512918728675-ed5a9ecdebfd_css5",
    "photo-1600566752355-35792bedcfea_css6",
]

# Let's clean up the pool so every element is a real distinct Unsplash ID:
CLEAN_POOL = []
for p in POOL:
    clean_id = p.split('_')[0]
    if clean_id not in CLEAN_POOL:
        CLEAN_POOL.append(clean_id)

print(f"Clean Unique Photo IDs count: {len(CLEAN_POOL)}")
